...
- [Latest relationship-preserving search and stopping decision](outputs/relational-key-findings.md): all three blind parameterized searches and an explicit pair-constraint trial time out. Complete affine spaces and the generated pair relation are independently checked. No Eye key; this feedback-family search is paused for a return to mechanism comparison.
- [Earlier combined passage and alphabet deductions](outputs/passage-domain-findings.md): with 27 supplied key entries, the combined filters recover the short squaring and inversion control keys where either filter alone stalls. Exact row and deletion certificates and held-out checks pass. All three extended blind searches exhaust their budgets; no Eye key.
...
cd Chesewip/outputs
..\..\.venv\Scripts\python.exe verify_relational_keys.py
