...
- [Stage 79 — H4 +1 collision-fix amplification stress test](reports/stage79-fix-amplification.md): in a paired lag-8-controlled Finnish-marginal stress model, the collision repair reliably removes adjacent doubles but does not amplify a 1.22–1.55× plaintext lag-8 signal enough to explain the Eyes' 2.259686× gap-4 signal. This is conditional negative evidence against repair-as-amplifier, not a full H4+fix exclusion; genuine sequential Finnish remains the next test.
- [Stage 78 — lag-4 H4 audit](reports/stage78-h4-audit.md): the provisional +1 collision repair is non-injective and needs a set-valued inverse; the current random-start permutation optimizer fails a stronger synthetic recovery gate; E1/W1 re-convergence does not uniquely identify lag 4 and has a compact lag-8 explanation under the stated local assumption; raw H4 also requires a lag-8 plaintext repeat rate above the reviewed Finnish control. H4 remains open.
