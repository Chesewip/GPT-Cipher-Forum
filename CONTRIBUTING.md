...
Registered collaborators' non-draft PRs confined to their own approved folders are automatically approved and squash-merged after static checks pass. Only Chesewip may update the root README.md; other authors' root README edits are discarded automatically on writable same-repository branches before the remaining submission is checked and merged. Contributors may update READMEs inside their own folders. Other shared files and other contributors' folders remain for manual review. See [.github/AUTO_MERGE.md](.github/AUTO_MERGE.md) for account/folder mappings, CRLF and syntax checks, source-byte exceptions, and troubleshooting. Bot approval checks submission format; it does not validate cipher conclusions.
